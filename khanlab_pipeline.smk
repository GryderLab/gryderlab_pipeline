...
onstart:
    shell("echo ' {pipeline_type} pipeline version {pipeline_version} started on Biowulf. Samples: {SAMPLES}. Working Dir:  {work_dir}' ") #|mutt -e 'my_hdr From:jxs1984@case.edu' -s 'Gryderlab {pipeline_type} Pipeline Status' `whoami`@case.edu {emails} ")
onsuccess:
    shell("echo ' {pipeline_type} pipeline version {pipeline_version} finished on Biowulf. Samples: {SAMPLES}. Working Dir:  {work_dir}' ") #|mutt -e 'my_hdr From:jxs1984@case.edu' -s 'Gryderlab {pipeline_type} Pipeline Status' `whoami`@case.edu {emails} ")
